...
class GeoFencing : AppCompatActivity(), OnMapReadyCallback, GoogleMap.OnPolygonClickListener,GoogleMap.OnMapClickListener {
    ...
    private fun savePolygonToDatabase(points: MutableList<LatLng>) {
        coroutineScope.launch {
            try {
                ...
                withContext(Dispatchers.Main) {
                    Toast.makeText(this@GeoFencing, "Polygon saved to DynamoDB", Toast.LENGTH_SHORT).show()
                }
            } catch (e: Exception) {
                Log.e("DatabaseHelper", "Error saving to DynamoDB", e)
                withContext(Dispatchers.Main) {
                    Toast.makeText(this@GeoFencing, "Error saving to DynamoDB: ${e.message}", Toast.LENGTH_LONG).show()
                }
            }
        }
    }
